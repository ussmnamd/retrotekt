...
export default function ProjectClientStandard({ project }: { project: Project }) {
  ...
  const { construction, renders } = assets;

  // Other projects for cross-linking (exclude current)
  const otherProjects = projects.filter((p) => p.slug !== project.slug);

  // Lightbox: separate state for renders vs construction
  const [rendersLightboxIndex, setRendersLightboxIndex] = useState<number | null>(null);
  const [constructionLightboxIndex, setConstructionLightboxIndex] = useState<number | null>(null);
  ...
  // Livermore: 6 photos → 3-col (2 rows of 3)
  // Sacramento: 10 photos → 5-col (2 rows of 5) — actual count drives this
  ...
  return (
    <div ref={root} className="bg-background min-h-screen text-primary">
      ...
      >
        ...
        {/* Dark gradient overlay — keep dark for text contrast over imagery */}
                  ...
                  <dd>
                    <ul className="space-y-1">
                      {project.scope.map((s) => (
                        <li
                          key={s}
                          className="font-body text-[13px] text-primary/70 flex items-center gap-2"
                        >
          ...
          <div className="max-w-[1400px] mx-auto">
            ...
            {renders[0] && (
              <div
                ...
                onClick={() => setRendersLightboxIndex(0)}
                role="button"
                tabIndex={0}
                onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setRendersLightboxIndex(0); }}
              ...
              <div data-anim="renders-grid" className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {renders.slice(1).map((img, i) => (
                  <div
                    ...
                    style={{ aspectRatio: '16/9' }}
                    onClick={() => setRendersLightboxIndex(i + 1)}
                    role="button"
                    tabIndex={0}
                    onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setRendersLightboxIndex(i + 1); }}
            ...
            <div className={`grid ${constructionCols} gap-3`}>
              {construction.map((img, i) => (
                <div
                  ...
                  onClick={() => setConstructionLightboxIndex(i)}
                  role="button"
                  tabIndex={0}
                  onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setConstructionLightboxIndex(i); }}
                  aria-label={`View ${img.alt} fullscreen`}
                >
      ...
      {/* ── 5. SIDE-BY-SIDE COMPARISON (static) ──────────────────────────── */}
      ...
      {/* Lightboxes */}
      {rendersLightboxIndex !== null && (
        <Lightbox
          images={renders}
          startIndex={rendersLightboxIndex}
          onClose={() => setRendersLightboxIndex(null)}
        />
      )}
      {constructionLightboxIndex !== null && (
        <Lightbox
          images={construction}
          startIndex={constructionLightboxIndex}
          onClose={() => setConstructionLightboxIndex(null)}
        />
      )}
